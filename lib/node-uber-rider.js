...
function Uber(userOBJ)
{
    ...
    this.access_token = userOBJ.access_token || "No Access Token Found";   
    this.refresh_token = userOBJ.refresh_token || "No Refresh Token Found";
    this.selfScopes = userOBJ.scope;  
}
...
{   
    ...
    request(options,function(error,response,body)
    {
        ...
        if(jsonRes["message"] == 'No authentication provided.')
        {
            //console.log('Invalid OAuth 2.0 credentials provided.');
            return callback("Error: "+'No authentication provided.',null);
        }
        //console.log(jsonRes);
...
Uber.prototype.getAuthorizeUrl = function(scopes)
{
...
{
    requestURL('/places/'+placeId,'',this.access_token,function(err,data){
        if(err){
            return callback('There was an error: '+err,null);
        }
        return callback(null,data);
